...
process receiver_3() {
	...
	umsg32 msg = proc->prmsg;
	if(msg != 0x87654321) {
		kprintf("Failed in first instance");
		kprintf("%s\tTest failed!%s\n", RED, RESET);
		return SYSERR;
	}
	...
	msg = proc->prmsg;
	if(msg != 0x90abcdef) {
		kprintf("Failed in 2nd instance");
		kprintf("%s\tTest failed!%s\n", RED, RESET);
		return SYSERR;
	}
	...
	msg = proc->prmsg;
	if(msg != 0x99990000) {
		kprintf("Failed in 3rd instance");
		kprintf("%s\tTest failed!%s\n", RED, RESET);
		return SYSERR;
	}
	...
	if(! proc->hasblockedsender || sender->prstate != PR_SENDING ||
		proc->prmsg != 0x25262728) {
			kprintf("Failed in 4th instance");
			kprintf("%s\tTest failed!%s\n", RED, RESET);
			return SYSERR;
	}
	...
	if(proc->hasblockedsender || sender->prstate != PR_READY) {
		kprintf("Failed in 5th instance");
		kprintf("%s\tTest failed!%s\n", RED, RESET);
		return SYSERR;
	}
	...
	msg = proc->prmsg;
	if(msg != 0x25262728) {
		kprintf("Failed in 6th instance");
		kprintf("%s\tTest failed!%s\n", RED, RESET);
		return SYSERR;
	}
	...
	if(msg != 0xbeefcafe) {
		...
		kprintf("checcking message 0xbeefcafe should be: %d\n",0xbeefcafe );
		kprintf("Failed in 7th instance\n");
		kprintf("%s\tTest failed!%s\n", RED, RESET);
		return SYSERR;
	}
	if(proc->hasblockedsender || sender->prstate != PR_READY) {
		kprintf("Failed in 8th instance");
		kprintf("%s\tTest failed!%s\n", RED, RESET);
		return SYSERR;
	}
